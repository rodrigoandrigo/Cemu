...
// Minecraft: Wii U Edition v80 profile Mii lookup. This version predates the
// community v688 Mii crash workaround and dereferences a null entry when the
// console FFL archive is not present. The signature is scoped to the title
// below and lets us select the first entry already produced by LoadAllData.
uint8 minecraftV80_miiLookupSignature[] = {
	0x38,0xA0,0x00,0x00, // li r5, 0
	0x7C,0xA7,0x2B,0x78, // mr r7, r5
	0x48,0x00,0x00,0x28, // b loop condition
	0x1C,0xC7,0x00,0x3C, // mulli r6, r7, 0x3c
	0x7D,0x88,0x32,0x14, // add r12, r8, r6
	0x38,0x9F,0x01,0xE4, // addi r4, r31, 0x1e4
	0x38,0x6C,0x00,0x2A, // addi r3, r12, 0x2a
	0x4B,0xFF,0xEF,0xCD, // bl ACTUuid::operator==
	0x2C,0x03,0x00,0x00, // cmpwi r3, 0
	0x41,0x82,0x00,0x08, // beq next entry
	0x7C,0xA8,0x32,0x14, // add r5, r8, r6
	0x38,0xE7,0x00,0x01  // addi r7, r7, 1
};
...
void GamePatch_scan()
{
	...
	// Minecraft v80 has no compatible community Mii crash patch. Without the
	// dumped Wii U FFL archive it still creates fallback entries, but searches
	// past all of them for an ACT UUID and then dereferences null. Select the
	// first generated entry instead. This preserves the normal copy and
	// sign-in path and does not weaken guest-memory validation globally.
	if (CafeSystem::GetForegroundTitleId() == 0x00050000101D9D00ULL)
	{
		hleAddr = hle_locate(minecraftV80_miiLookupSignature, nullptr, sizeof(minecraftV80_miiLookupSignature));
		if (hleAddr)
		{
			// Always accept the first entry and leave the search loop.
			memory_writeU32(hleAddr + 0x24, 0x60000000); // nop (was beq next)
			memory_writeU32(hleAddr + 0x2C, 0x4800003C); // b copy-and-sign-in
			cemuLog_log(LogType::Force,
				"Minecraft v80: enabled missing-FFL Mii fallback at 0x{:08x}", hleAddr);
		}
	}
